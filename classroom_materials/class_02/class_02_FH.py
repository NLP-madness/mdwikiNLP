...
<<<<<<< HEAD:classroom_materials/class_02/class_02_VMP.py
...
>>>>>>> cd586459128d06a65862cb3b268fb2abb863ed22:classroom_materials/class_02/class_02_FH.py
    """
    tokenlist (list): A list of tokens
...
def postag_stanza(tokenlist, return_df = False):
    ...
    import stanza
    nlp = stanza.Pipeline(lang='en', processors='tokenize,lemma,mwt,pos', tokenize_pretokenized=True)
    doc = nlp(tokenlist)

    res = [(word.lemma, word.pos)
           for n_sent, sent in enumerate(doc.sentences)
           for word in sent.words]
