...
if __name__ == "__main__":
    ...
    X_train_counts = count_vect.fit_transform(X_train)  # hashing, train/transform.
    print(type(X_train_counts))  # sparse matrix (fast?)

    # TASK 2: What does the shape of the X_train_counts denote
    print(X_train_counts.shape)
    # hint 1 what do get if you run:

    bow = count_vect.transform(
        ["this is a test"]
    )  # one word letters/digits --> not stop?
    print(f"bow = {bow}")
    bow.todense()  # this right here transform the sparse matrix to a dense matrix

    # hint 2: what is the shape of the dense matrix?
    print(f"bow.shape = {bow}")
    print(f"bow.shape = {bow.shape}")

    # Train
    clf = MultinomialNB()  # new class / instance.
    clf.fit(X_train_counts, y_train)  # y = class.
